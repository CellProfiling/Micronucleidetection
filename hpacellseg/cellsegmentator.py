...
class CellSegmentator(object):
    ...
    def label_nuclei(self, images, generator=False):
        ...
        def _postprocess(n_prediction):
            ...
            mask_img = skimage.morphology.remove_small_objects(mask_img, 10000)
            mask_img = mask_img.astype(np.uint8)
            nuclei_label = skimage.segmentation.watershed(mask_img, markers,
                                                          mask=mask_img,
                                                          watershed_line=True)
            return nuclei_label
    ...
    def restore_scaling_padding(self, n_prediction):
        ...
        if not self.scale_factor == 1:
            ...
            n_prediction = cv2.resize(n_prediction, (self.target_shape[0], self.target_shape[1]),
    ...
    def label_cells(self, images, generator=False):
        ...
        def _preprocess(image):
            ...
            if self.padding:
                ...
                self.scaled_shape = rows, cols
                cell_image = cv2.copyMakeBorder(cell_image, 32, (32-rows % 32), 32, (32-cols % 32),
                                                cv2.BORDER_REFLECT)
